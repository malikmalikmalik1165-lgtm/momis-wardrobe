...
export default function AdminGuidePage() {
  ...
  const sections = [
    ...
    {
      icon: "📥", title: "Excel Se Bulk Import (100+ Products Ek Baar)",
      steps: [
        "1️⃣ Admin → '📦 Products' tab → '📥 Import Excel' button tap karein",
        "2️⃣ Do raste hain:",
        "   → A) '⬇ Download Template' — momis-products-template.xlsx mein apne products likhein",
        "   → B) Markaz/WooCommerce EXPORT FILE seedha drop karein (.csv/.xlsx) — format khud pehchana jata hai, columns khud map hote hain (Name, Sale price → live price + regular price → compare-at, Images URLs, Categories, In stock?, Size/Colour attributes)",
        "",
        "📋 TEMPLATE COLUMNS:",
        "   → Name* — Product ka naam (ZAROORI)",
        "   → Price* — Aapki bechne wali price (ZAROORI)",
        "   → ComparePrice — Purani price, sale dikhane ke liye",
        "   → Category — Suits, Lawn, Kurti... (naya naam likhein to category auto-ban jayegi)",
        "   → Description — Product details",
        "   → Sizes — S, M, L, XL (comma se alag)",
        "   → Colors — Black, Red (comma se alag)",
        "   → Images — Image URLs, comma se alag (multiple ho sakti hain)",
        "   → Badge — New, Sale, Best Seller",
        "   → Featured — Yes/No (Home page par feature hoga)",
        "   → InStock — Yes/No",
        "   → SKU — Apna code likhein ya blank chhor dein (auto MW-0001 banega)",
        "",
        "3️⃣ Apne saare products template mein likhein (Markaz/Daraz se copy-paste kar sakte hain) — ya WooCommerce/Markaz export file as-is rakhein",
        "4️⃣ Import modal mein file select karein ya drag & drop karein (.xlsx, .xls, .csv)",
        "5️⃣ PREVIEW dikhega — detected format, kitni rows import hongi, kaun si skip hongi, har row ki images count",
        "6️⃣ 'Import' button tap karein → ✅ result message aayega",
        "",
        "💡 TIPS:",
        "   → File mein koi field miss ho jaye to preview mein us row par ✏️ Fill dabayen aur khud bhar lein — row skip nahi hogi",
        "   → Images ki URLs https:// honi chahiye (kisi bhi website/CDN ki chalegi) — isi liye pics website par show hoti hain",
        "   → Pehle se imported products ki pics show na hon to usi file ko dobara import karein — SKU match ho kar images update ho jayengi",
        "   → Google Sheets se bhi import kar sakte hain: File → Download → Excel (.xlsx)",
        "   → Momis template ke columns exact chahiye; Markaz/WooCommerce export columns auto-detect ho jate hain",
        "   → Markaz/Woo export mein 'Sale price' = aapki live price, regular 'Price' = compare-at (strikethrough) ban jati hai",
        "   → Published=0 ya hidden rows, aur bina Name/Price wali rows skip ho jayengi (preview mein pehle dikhti hain)",
        "   → Same file dobara import karne par duplicate NAHI banega — SKU matching products update ho jate hain",
        "   → Pehle 2 sample rows template mein hain — unko delete karke apna data likhein",
      ],
    },
    {
